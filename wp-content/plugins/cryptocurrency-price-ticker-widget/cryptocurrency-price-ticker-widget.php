...
*/
define( 'Crypto_Currency_Price_Widget_VERSION', '1.7' );
...
final class Crypto_Currency_Price_Widget {
	...
	$dynamic_styles.=".tickercontainer #".$id."{".$bg_color."}
                     .tickercontainer span.name #".$id."{".$fnt_color."}	
                      .tickercontainer span.coin_symbol #".$id."{".$fnt_color."}
                        
                      .tickercontainer span.price #".$id."{".$fnt_color."} .tickercontainer .price-value{".$fnt_color."}
	...
	$dynamic_styles_multicurrency.=".currency_tabs#".$id.",.currency_tabs#".$id." ul.multi-currency-tab li.active-tab{".$bg_color."border-color".$fnt_coloronly."}
									...
									.currency_tabs#".$id." ul.multi-currency-tab-content li{border-color".$fnt_coloronly."}";
					  
	$dynamic_styles_multicurrency.=$custom_css;
	...
	// fetching data from api 
		$all_coin_data=$this->get_api_data(150);
		...
		if( ! empty( $all_coin_data ) && is_array($all_coin_data)) {
			...
			if(is_array($display_currencies)&& !empty($display_currencies)){
				...
				if(in_array($coin->id,$display_currencies)){
					$crypto_html .=$this->generate_html($coin,$type,$display_logo,$display_changes,$ticker_position,$header_ticker_position,$ticker_speed,$back_color,$font_color,$custom_css,$usd_conversions);
				}
			}	
			}else{
		...
		if ($type=="ticker"){
			...
			$output .= '</tbody></table></div>';
				...
				$styles="<style type='text/css'>".$dynamic_styles_multicurrency."</style>";
...
function generate_html($coin,$type,$display_logo,$display_changes,$ticker_position,$header_ticker_position,$ticker_speed,$back_color,$font_color,$custom_css,$usd_conversions){
			$coin_html='';
		...
		$euro_price=ccpwt_format_number($coin_price*$EUR);
		$gbp_price=ccpwt_format_number($coin_price*$GBP);
		$aud_price=ccpwt_format_number($coin_price*$AUD);
		$jpy_price=ccpwt_format_number($coin_price*$JPY);

		$usd_price=ccpwt_format_number($coin_price);
			$coin_html.=  '<li id="'.esc_attr( $coin_id).'">';
				...
				<span data-aud="'.$aud_price.'" data-jpy="'.$jpy_price.'" data-gbp="'.$gbp_price.'" data-eur="'.$euro_price.'" data-usd="'.$usd_price.'" class="mtab_price">' . $coin_price . '</span>';
